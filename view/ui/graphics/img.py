...
class Img:
    ...
    def draw_rect(self, x: int, y: int, w: int, h: int,
                  color=(255, 255, 255, 255), thickness: int = -1,
        ...
        color   : (B, G, R, A) tuple
        thickness : -1 = filled, >0 = border only
        alpha   : 0.0–1.0 blend factor for filled rectangles (semi-transparent)
        """
        if self.img is None:
            raise ValueError("Image not loaded.")
        b, g, r = color[0], color[1], color[2]
        if thickness == -1 and alpha < 1.0:
            overlay = self.img.copy()
            cv2.rectangle(overlay, (x, y), (x + w, y + h), (b, g, r), -1)
            cv2.addWeighted(overlay, alpha, self.img, 1.0 - alpha, 0, self.img)
        else:
            cv2.rectangle(self.img, (x, y), (x + w, y + h), (b, g, r), thickness)
